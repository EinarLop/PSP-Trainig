...
public class FinalOperations {
    ...
    public double[] setupResults() {
        ...
        for (int i = 0; i < finalOperationsResults.length; i++) {
            String temp = formatter.format(finalOperationsResults[i]).replace(",", ".");
            Double parsedTemp = Double.parseDouble(temp);
            finalOperationsResults[i] = Math.round(parsedTemp * 100000.0) / 100000.0;
        }
